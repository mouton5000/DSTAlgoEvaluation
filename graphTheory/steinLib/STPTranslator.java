...
/**
 * 
 * This class contains static method to translate .stp files into a {@link SteinerInstance},
 * directed or not, and to translate a {@link SteinerInstance} into a .stp file.
 * 
...
public class STPTranslator {
	...
	/**
	 * Return the {@link SteinerDirectedInstance} or the {@link SteinerUndirectedInstance}
	 * conresponding to the .stp file in input.
	 * If the file describes a graph containing directed arcs and undirected edges, then
	 * this method returns null. 
	 * 
	 * @param nomFic path of the file describing the instance we want to build.
	 * @return 
	 * @throws STPTranslationException 
	 */
	public static SteinerInstance translateFile(String nomFic) throws STPTranslationException {
		...
		if (s == null){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.EMPTY_FILE, 
					nomFic, lineNumber, null);
		}
		...
		if (!s.contains("33d32945")){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.BAD_FORMAT_CODE, 
					nomFic, lineNumber,s);
		}
		...
		while (!s.contains("section graph")) {
			...
			if (s == null){
				throw new STPTranslationException(
						STPTranslationExceptionEnum.NO_SECTION_GRAPH, 
						nomFic, lineNumber,s);
			}
			s = s.toLowerCase();
		}
		...
		if (s == null){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.EMPTY_SECTION_GRAPH, 
					nomFic, lineNumber,s);
		}
		s = s.toLowerCase();
		s = s.trim();
		...
		if (!m.matches())
			throw new STPTranslationException(
					STPTranslationExceptionEnum.NODE_NUMBER_BAD_FORMAT, 
					nomFic, lineNumber,s);
		...
		if (s == null){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.EDGE_NUMBER_BAD_FORMAT, 
					nomFic, lineNumber,s);
		}
		s = s.toLowerCase();
		s = s.trim();
		...
		if (noe != 0){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.INCOHERENT_NB_EDGES, 
					nomFic, lineNumber,s);
		}
		...
		while (!s.contains("section terminals")) {
			...
			if (s == null){
				throw new STPTranslationException(
						STPTranslationExceptionEnum.NO_SECTION_TERM, 
						nomFic, lineNumber,s);
			}
			s = s.toLowerCase();
		}
		...
		if (s == null){
			throw new STPTranslationException(
					STPTranslationExceptionEnum.EMPTY_SECTION_TERM, 
					nomFic, lineNumber,s);
		}
		s = s.toLowerCase();
		...
		if (m.matches()) {
			...
			if (not > size|| not <= 0){
				throw new STPTranslationException(
						STPTranslationExceptionEnum.STRANGE_NB_TERM, 
						nomFic, lineNumber,s);
			}
		} else{
			throw new STPTranslationException(
			...
			if (m.matches()) {
				if (m.group(3) != null) {
					if (rootSet || !isDirected) {
						...
						rootSet = true;
						((SteinerDirectedInstance) g).setRoot(Integer
								.valueOf(m.group(4)));
					}
				} else {
		...
		if (not != 0) {
			throw new STPTranslationException(
					STPTranslationExceptionEnum.INCOHERENT_NB_TERMS, 
					nomFic, lineNumber,s);
		}
	...
	/**
	 * 
	 * Return the {@link SteinerDirectedInstance}  conresponding to the .stp file in input.
	 * If the file describes a graph containing undirected edges, then
	 * this method returns null. 
	 * 
	 * @param nomFic path of the file describing the instance we want to build.
	 * @return 
	 * @throws STPTranslationException 
	 */
	public static SteinerDirectedInstance translateDirectedFile(String nomFic) throws STPTranslationException {
	...
	/**
	 * 
	 * Return the {@link SteinerUndirectedInstance}  conresponding to the .stp file in input.
	 * If the file describes a graph containing directed arcs, then
	 * this method returns null. 
	 * 
	 * @param nomFic path of the file describing the instance we want to build.
	 * @return 
	 * @throws STPTranslationException 
	 */
	public static SteinerUndirectedInstance translateUndirectedFile(String nomFic) throws STPTranslationException {
		SteinerInstance g = translateFile(nomFic);
	...
	public static void translateSteinerGraph(SteinerInstance g, String nomFic,
		...
		f.writeln((isDirected ? "Arcs" : "Edges") + " " + g.getGraph().getNumberOfEdges());
