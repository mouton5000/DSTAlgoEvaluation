...
/**
 * Implementation of the dijkstra algorithm. This algorithm compute the shortest path
 * between all the nodes to a destination in a directed or undirected graph. 
 * 
 ...
 *
 */
public class ArcDijkstraOneDestinationAlgorithm extends Algorithm {

	
	/**
	 * Input instance
	 */
	protected ArcShortestPathOneDestinationInstance instance;
	
	/**
	 * If true, do not compute the shortest paths, but only the costs of the shortest paths
	 */
	protected boolean computeOnlyCosts; 
	
	/**
	 * For each node v, this map contains the shortest path from v to the destination
	 */
	protected HashMap<Integer,List<Arc>> shPs;
	
	/**
	 * For each node v, this map contains the cost of the shortest path from v to the destination
	 */
	protected HashMap<Integer,Integer> costs;



	public ArcDijkstraOneDestinationAlgorithm() {
		super();
	}
	...
	public void setInstance(ArcShortestPathOneDestinationInstance instance) {
	...
	/**
	 * Init the distance from every node to the destination: 
	 * the destination is at distance 0 from the destination and every other node
	 * is at infinite distance until they are reached.
	 */
	...
	/**
	 * See if using a path of weight distanceToDestination, and the arc a, we can 
	 * reduce the distance from the node input to the destination. 
	...
	private void expand(Integer distanceToDestination, Arc a, Integer input){
		...
		if((nndN == null || nndN > dist)){
			FibonacciHeap<Integer>.FibonacciHeapNode<Integer> fhn = nodes.get(input);
	...
	/**
	 * Using the map preceding compute and return the lowest cost
	 * path from  all nodes to the destination
	...
	/**
	 * Using the map preceding compute and return the lowest cost
	 * path from  v to the destination
