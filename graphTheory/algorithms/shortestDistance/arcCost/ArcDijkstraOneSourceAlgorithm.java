 ...
 * between a source and all the nodes in a directed or undirected graph. 
 * 
...
public class ArcDijkstraOneSourceAlgorithm extends Algorithm{
	...
	/**
	 * For each node v, this map contains the cost of the shortest path from the source to v
	 */
	...
	/**
	 * Init the distance from the source to every node : 
	 * the source is at distance 0 from the source and every other node
	...
	private void expand(Integer distanceFromSource, Arc a, Integer output){
		...
		if((nndN == null || nndN > dist)){
			FibonacciHeap<Integer>.FibonacciHeapNode<Integer> fhn = nodes.get(output);
