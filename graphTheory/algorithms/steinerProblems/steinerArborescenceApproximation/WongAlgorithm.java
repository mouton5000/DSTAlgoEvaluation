 ...
 * 
 * Source : 
 * The algorithm is described in R. Wong, "A dual ascent approach for Steiner Tree Problems on a directed graph".
 * But the source used was the description in M. P. de Aragao : "Dual Heuristics on the Exact Solution of Large Steiner problems". 
 * 
 * The algorithm search, from the terminals, for a path to the root by saturating arcs. Each arc is associated with a reduced cost
 * initiated to the steiner cost of the arc. An arc with a 0 reduced cost is saturated. 
 * A component is one of the maximal (not necessarily strongly) connected component reaching one or more terminals with saturated arcs.
 * At the begining, if the graph do not contains arcs with cost 0, the first components are the singletons containing each terminals.
 * 
 *  Each turn, we consider every components not containing the root. If there is no, we build a tree with the saturated arcs.
 *  If there are some, we compute the minimum reduced cost C over all the entering arcs over all the components (an entering arc
 *  is an arc with input outside and output inside). 
 *  If a is the arc entering the component Comp and is choosed, we then reduce the reduced cost of every of 
 *  the comp entering arcs by C (at least one arc become saturated).
 *  And we continue with the new components until each terminals is reached by the root with saturated arcs.
 *  
 *  Notice that each turn, we add in fact C to the dual solution of this steiner problem. 
 *  As a consequence, we easily get a lower bound to the Steiner problem. This
 *  lower bound is registered as an output of the algorithm. The dual solution is also registered.
 *  
...
public class WongAlgorithm extends SteinerArborescenceApproximationAlgorithm{
	...
	/**
	 * Costs of the arcs described by the variable "d"
	 * in Wong "A dual ascent approach for Steiner Tree problems on a directed Graph"
	 */ 
	private HashMap<Arc, Double> reducedCost;


	/**
	 * For each couple of nodes of index i and j, 
	 * connections[i](j] is true if there is a path from the first
	 * to the second node in the auxilliary graph.
	 */
	...
	private void setSolution(DirectedGraph gp) {
		...
		MinimumBranchingArborescenceInstance mbai = new MinimumBranchingArborescenceInstance(gp);
	...
	/**
	 * Step 2 in Wong "A dual ascent approach for Steiner Tree problems on a directed Graph".
	 * Edit gp. h is a root component of gp. a = (i*,j*) is the minimum cost arc entering h.
	 * gp contains a path from j* to i*.
	 * @param gp
	 * @param h
	 * @param a
	 */
	 ...
	 * is not is that component. There cannot be any node n which can reach the component
	 * such that no node of the component can reach n.
	 * That component do not contains the root.
	 * 
	 * 
	 * @param gp
	 * @return
