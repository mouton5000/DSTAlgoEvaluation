...
/**
 * 
 * Implementation of the Roos modified algorithm.
 * Source : "FasterDSP: A faster approximation algorithm for directed Steiner tree problem"; by Hsieh, Ming-I et Al.
 * <p>
 * It is a faster implementation for the Charikar and Al. algorithm from "Approximation Algorithms for Directed Steiner Tree Problems"
 * when the height parameter is equals to 2.
 * <p>
 * This algorithm returns a feasible solution for the Directed Steiner Tree problem.
 * It is a greedy algorihm which uses a sub-algorihthm named CH2 designed to returns a feasible solution 
 * for the minimum Density Directed Steiner Tree problem: it searches for a tree rooted in the root of the instance which 
 * spans a part of the terminal, such that the height of the tree is at most 2. 
 * It try to minimize the total cost of the tree divided by the number of terminals.
 * <p>
 * The roos algorithm uses CH2 to span some terminals, delete those terminals, and repeat CH2 to span other terminals.
 * It continues until all the terminals are reached.
 * <p>
 * Notice CH2 search for the tree of height 2 in the shortest path graph : the complete directed graph where the wieght of the
 * arc is the shortest path cost in the original graph.
 * <p>
...
public class RoosAlgorithm  extends SteinerArborescenceApproximationAlgorithm{
	...
	/**
	 * This map associates each couple of node (represented  by an arc in the original graph)
	 * to a list of arcs in the original graphs corresponding to the shortest path  between
	 * the two nodes.
	 * <p>
	 * Notice that not all the couple of nodes are registered, indeed, we just need the couple
	 * (root, v) for each node v, and (v,t) for each node v and each terminal t.
	 */
	private HashMap<Arc, List<Arc>> shortestPaths;
	
	/**
	 * This map associates for each node v the list of terminals sortest by distance from v :
	 * the cost of the shortest path from v to that terminal.
	 */
	...
	protected void computeWithoutTime() {
		...
		HashSet<Integer> req = new HashSet<Integer>(instance.getRequiredVertices());
	 ...
	 * {@link #shortestPaths} map. Register also their cost in the {@link #costs} map.
	 */
	private void initShortestPathsFromRoot() {
		...
		// from a single node
		ArcShortestPathOneSourceInstance ash = new ArcShortestPathOneSourceInstance(instance.getGraph());
	...
	/**
	 * Compute all the shortest paths to all terminals and registered them in the 
	 * {@link #shortestPaths} map. Register also their cost in the {@link #costs} map.
	 */
	private void initShortestPathsToRequiredVertices(){
		...
		ArcShortestPathOneDestinationInstance ash = new ArcShortestPathOneDestinationInstance(instance.getGraph());
		ash.setCosts(instance.getCosts(true));
	...
	private void sortRequiredVertices(Integer v){
		...
		// of each terminal
		TreeSet<Integer> ts = new TreeSet<Integer>(getRequiredVerticesComparator(v));
		while(it.hasNext()){
			Integer t = it.next();
			ts.add(t);
		}
	 ...
	 * 
	 * @param v
	 * @return a comparator which compares two terminals by distance from v: the closest terminals
	 * to v is the lowest.
	 */
	...
	/**
	 * @param 
	 * @return a tree rooted in the root of the instance spanning a part of the terminals in req, 
	 * and the set of those terminals.
	 */
