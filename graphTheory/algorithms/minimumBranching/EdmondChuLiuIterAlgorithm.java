...
/** 
 * The Edmond algorithm, or the Chu Liu Algorithm, compute in a directed graph
 * a minimum branching tree, equivalent to the minimum spanning tree in a directed
 * graph. Given a root r and weight on the arcs, the algorithm compute the minimum
 * weighted directed tree rooted in r spanning every nodes of the graph.
 * 
 ...
 * 
 * Replace that cycle by a node vc. Every input arc a = (u,v) in the cycle becomes an
 * input arc of vc with modified cost. The modified cost is equals to the cost of
 * a minus the cost of v chosen arc (belonging to the cycle). Every output arc of the cycle becomes  
 * an output arc of vc with same cost. Every arc where input and output belongs to
 * the cycle does not appear in that new graph.
 *
 * Restart the algorithm on that new graph, until there is no cycle.
 * 
 * Once a tree T is returned, backtrack on every contracted cycle. Replace the node
 * vc by the corresponding cycle. Add every arc of the cycle. Every arc of T incomming or outgoing from vc is 
 * replaced by the corresponding arc incomming or outgoing from the cycle. If an
 * incomming arc (u,v) is added to T, then remove the v choosen arc from the cycle and T.
 *
 * Contrary to the other verion of Edmond Chu Liu, this version is iterative. But it modifies the graph during
 * computation by adding the contracted nodes and then deleting them at the end of the algorithm.
...
public class EdmondChuLiuIterAlgorithm extends MinimumBranchingAlgorithm {
	...
	/**
	 * Create a new EdmongChuLiu algorithm. Must precise the instance, root and arc costs
	 * before computing.
	 */
	...
	/**
	 * Costs of the arcs added during the algorithm (linked to the contracted nodes)
	 */ 
	...
	/**
	 * List of couples linking every contraction node to the corresponding cycle. 
	 */
	private LinkedList<Couple<Integer,HashSet<Integer>>> contractions;

	/**
	 * Virtually remove every node and arc in the cycle, and add a new node replacing
	 * the cycle. Each ingoing or outgoing arc in or out the cycle are copied
	 * as an arc ingoing or outgoing this new node.
	 */
	private void contractCycle(HashSet<Integer> cycle){
		...
		contractions.addFirst(new Couple<Integer, HashSet<Integer>>(contractNode, cycle));
	}
	 ...
	 * with modified cost :  cost(u,v) - cost(v input arc in the cycle).
	 * If multiple arcs (u,*) enter the cycle, chose the one with minimum
	 * modified cost. 
	 * 
	 * @param contractNode
	 * @param cycle
	 */
	private void setContractionInputs(Integer contractNode, HashSet<Integer> cycle){
	 ...
	 * with same cost.
	 * If multiple arcs (*,u) out from the cycle, chose the one with minimum
	 * cost. 
	 * 
	 * @param contractNode
	 * @param cycle
	 */
	private void setContractionOutputs(Integer contractNode, HashSet<Integer> cycle){
	...
	/**
	 * Backtrack on every contracted cycle to :
	 * - remove the added contraction node vc
	 * - replace every arc entering or outgoing from vc by
	 * the corresponding arc entering or outgoing from  the cycle
	 * @param h
	 */
	private void returnCyclesBack(HashSet<Arc> h) {
		for(Couple<Integer, HashSet<Integer>> contraction : contractions){
			...
			Iterator<Arc> it = instance.getGraph().getInputArcsIterator(contractNode);
			while(it.hasNext()){
				Arc b = it.next();
				if(!h.contains(b))
					continue;
